...
from store.models import Product, ProductImage, SubCategory
...
print("=== Fixing Men's Ethnic Wear Images (Existing Subcategories) ===")
...
ethnic_wear_products = Product.objects.filter(
    category__name='Men'
).filter(subcategory__name__in=ethnic_subcategories)

print(f"Found {ethnic_wear_products.count()} ethnic wear products")
