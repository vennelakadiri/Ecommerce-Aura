...
# All custom URLs specified across user messages
USER_SPECIFIED_URLS = {
...
def apply_exact_user_urls():
    print("=" * 60)
    print("REPLACING NAMED PRODUCT URLS WITH USER SPECIFIED URLS")
    print("=" * 60)
    
    updated = 0
    for name, target_url in USER_SPECIFIED_URLS.items():
        products = Product.objects.filter(name__iexact=name, is_active=True)
        if not products.exists():
            products = Product.objects.filter(name__icontains=name, is_active=True)
        ...
        for p in products:
            ...
            updated += 1
            print(f"  [Product ID {p.id:4d}] '{p.name}' -> {target_url}")
            
    print("=" * 60)
    print(f"SUCCESSFULLY UPDATED {updated} PRODUCTS WITH USER'S CUSTOM URLS.")
