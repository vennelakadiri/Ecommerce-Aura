...
def download_and_upload_to_cloudinary(image_url, product_name, category_name):
    """
    Download an image from URL and upload it to Cloudinary.
    """
    try:
        # Download the image
        ...
        if response.status_code == 200:
            ...
            upload_result = cloudinary.uploader.upload(
                ...
                public_id=f"{product_name.replace(' ', '_').lower()}",
                overwrite=True
            )
...
def get_category_image_url(category_name):
    """
    Get a working image URL for a category.
    """
    category_urls = {
...
def update_product_images_with_cloudinary():
    """
    Update all product images by downloading and uploading to Cloudinary.
    """
    ...
    for index, product in enumerate(products, 1):
        ...
        try:
            ...
            cloudinary_url = download_and_upload_to_cloudinary(
                image_url, 
                product.name, 
...
if __name__ == "__main__":
    print("=" * 60)
    print("UPLOAD PRODUCT IMAGES TO CLOUDINARY")
    print("=" * 60)
    print("This script will download category images and upload them")
    print("to Cloudinary, then update the product image references.")
    print("=" * 60)
    
    confirm = input("\nDo you want to continue? (yes/no): ")
    if confirm.lower() in ['yes', 'y']:
        update_product_images_with_cloudinary()
    else:
